...
UnboundedInt unboundedInt_divide(UnboundedInt a_ui, UnboundedInt b_ui) {
    ...
    long long cmpt = 0;
    UnboundedInt qo = unboundedInt_subtract(a, b);
    while (unboundedInt_cmpUnboundedInt(qo, b) != -1) {
        cmpt++;
    }
 ...
 */
static UnboundedInt sumNegatifUnbounded(UnboundedInt a, UnboundedInt b) {
    int plusgrand = unboundedInt_cmpUnboundedInt(a, b);
    UnboundedInt res = UNBOUNDED_INT_ERROR;
    if (plusgrand == 0) {
        res.mSign = '+';
        res = pushBack(res, '0');
...
static int plusGrandAbs(UnboundedInt a, UnboundedInt b) {
    a.mSign = '+';
    b.mSign = '+';
    return unboundedInt_cmpUnboundedInt(a, b);
}
